window.onload = function () {
    document.getElementById('jscalc').onclick = function() {
        ...
        function getTitle(title) {
            let TitleElement = document.querySelector("" + title + "").innerHTML.trim();
            result = TitleElement.charAt(0).toUpperCase() + TitleElement.slice(1).toLocaleLowerCase();
            if(result.length > 12) {
                result = result.slice(0,12) + '...';
            }
            document.querySelector("" + title + "").innerText = result;
        }
